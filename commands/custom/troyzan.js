...
const ags = message.content.slice(1).split(',');
  ...
  const messageforsend = message.content.substring(11)
	const canvas = Canvas.createCanvas(800, 804);
	const ctx = canvas.getContext('2d');
	const background = await Canvas.loadImage('./troyzan.png');
    ...
    ctx.rotate(-2.33 * Math.PI / 180);
    text = `${s1}`
    x = canvas.width / 2 - ctx.measureText(text).width / 2 - 25
    ...
    x = canvas.width / 2 - ctx.measureText(text).width / 2 - 25
    y = 675// - ctx.measureText(text).height / 2
